...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, saleCutPercents: {Address: UFix64}) {
    ...
    let nftProvider: Capability<&BlockleteGames_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(signer: AuthAccount) {
        ...
        let nftCollectionProviderPrivatePath = /private/blockleteGamesCollectionProviderForNFTStorefront
        if !signer.getCapability<&BlockleteGames_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!.check() {
            signer.link<&BlockleteGames_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath, target: BlockleteGames_NFT.CollectionStoragePath)
        }
        ...
        self.nftProvider = signer.getCapability<&BlockleteGames_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!
        assert(self.nftProvider.borrow() != nil, message: "Missing or mis-typed BlockleteGames_NFT.Collection provider")
    ...
    execute {
        self.storefront.createListing(
            ...
            nftType: Type<@BlockleteGames_NFT.NFT>(),
            nftID: saleItemID,
