...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, saleCutPercents: {Address: UFix64}) {
    ...
    let nftProvider: Capability<&NyatheesOVO.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(signer: AuthAccount) {
        ...
        let nftCollectionProviderPrivatePath = /private/nyatheesOVOCollectionProviderForNFTStorefront
        if !signer.getCapability<&NyatheesOVO.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!.check() {
            signer.link<&NyatheesOVO.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath, target: NyatheesOVO.CollectionStoragePath)
        }
        ...
        self.nftProvider = signer.getCapability<&NyatheesOVO.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!
        assert(self.nftProvider.borrow() != nil, message: "Missing or mis-typed NyatheesOVO.Collection provider")
