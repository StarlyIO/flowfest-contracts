...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, saleCutPercents: {Address: UFix64}) {
    ...
    let nftProvider: Capability<&RareRooms_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(signer: AuthAccount) {
        ...
        let nftCollectionProviderPrivatePath = /private/rareRoomsNFTCollectionProviderForNFTStorefront
        if !signer.getCapability<&RareRooms_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!.check() {
            signer.link<&RareRooms_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath, target: RareRooms_NFT.CollectionStoragePath)
        }
        ...
        self.nftProvider = signer.getCapability<&RareRooms_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!
        assert(self.nftProvider.borrow() != nil, message: "Missing or mis-typed RareRooms_NFT.Collection provider")
    ...
    execute {
        self.storefront.createListing(
            ...
            nftType: Type<@RareRooms_NFT.NFT>(),
            nftID: saleItemID,
