...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, saleCutPercents: {Address: UFix64}) {
    ...
    let nftProvider: Capability<&SportsIconCollectible.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(signer: AuthAccount) {
        ...
        let nftCollectionProviderPrivatePath = /private/sportsIconCollectibleCollectionProviderForNFTStorefront
        if !signer.getCapability<&SportsIconCollectible.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!.check() {
            signer.link<&SportsIconCollectible.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath, target: SportsIconCollectible.CollectionStoragePath)
        }
        ...
        self.nftProvider = signer.getCapability<&SportsIconCollectible.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!
        assert(self.nftProvider.borrow() != nil, message: "Missing or mis-typed SportsIconCollectible.Collection provider")
